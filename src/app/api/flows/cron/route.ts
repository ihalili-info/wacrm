 ...
 *
 * Auth: shared with `/api/automations/cron` via `authorizeCronRequest`
 * — `Authorization: Bearer ${CRON_SECRET}` from Vercel Cron, or
 * `x-cron-secret` matching `AUTOMATION_CRON_SECRET` from an external
 * scheduler — so operators only provision one secret. The two
 * endpoints are independent operations kept on separate URLs so one
 * failing doesn't block the other.
 *
 * Hosting: hit on a schedule (Vercel Cron — see `vercel.json` — /
 * GitHub Actions / external pinger). A 5-minute interval is more than
 * enough for a 24h timeout default; once per hour would also be
 * acceptable for low-volume tenants.
 */
