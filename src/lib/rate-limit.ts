 ...
 * fine for a single-instance deploy (a container on a VPS, one
 * long-lived Node process).
 *
 * Trade-off: a single process holds the Map, so anything that fans
 * requests across processes (Vercel serverless / multiple regions, a
 * load-balanced container pool) weakens the limit — each instance
 * enforces its own budget. If you need a hard global limit, swap the
 * `check` implementation for Redis / Upstash keeping the same return
 * shape. The call sites won't change.
 *
