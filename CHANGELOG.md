...
Switches the recommended host from Hostinger to **Vercel**, with the
database provisioned through Vercel's native **Supabase** integration.
...
- **Deploy docs are now Vercel-first.** New
  ...
  `supabase db push` → Meta webhook → cron). README's deploy section,
  badges, and the "deploy" button all point at Vercel. Hostinger
  marketing and the referral links are removed; the MIT license still
  lets you run it anywhere Node.js 20+ does
  (`npm run build && npm run start`).
- **Scheduled jobs support Vercel Cron natively.** A committed
  [`vercel.json`](./vercel.json) declares the two jobs
  (`/api/automations/cron` every 5 min, `/api/flows/cron` every
  10 min). Both endpoints now accept **either**
  `Authorization: Bearer ${CRON_SECRET}` (Vercel Cron) **or** the
  existing `x-cron-secret` header matching `AUTOMATION_CRON_SECRET`
  (external schedulers). Auth is centralised in
  `src/lib/cron/auth.ts`. New guide:
  [`docs/automations-and-cron.md`](./docs/automations-and-cron.md).
- Docstrings and one user-facing error string that named Hostinger
  are updated; no runtime behaviour change.

### Removed

- **All upstream `.github/` scaffolding** — CI workflows (`ci.yml`,
  `migrations.yml`), Dependabot, `CODEOWNERS`, issue templates, the PR
  template, `SECURITY.md`, and `CODE_OF_CONDUCT.md`. These assumed the
  upstream repo (auto-requesting `@ArnasDon` as reviewer, opening
  weekly dependency PRs) and get in the way of a fork. Add your own
  workflows and policies. `npm run typecheck` / `lint` / `test` /
  `build` are unchanged — wire them into whatever CI you set up.
- **Docker support** — `Dockerfile`, `docker-compose.yml`,
  `.dockerignore`, and `docs/docker.md`. `output: "standalone"` is
  dropped from `next.config.ts` (it only existed for the image). The
  app still runs on any Node.js 20+ host with
  `npm run build && npm run start`.

> **No migration required.** If you deploy on Vercel, add `CRON_SECRET`
> to your environment so the cron routes accept Vercel's calls; the
> `AUTOMATION_CRON_SECRET` path is unchanged for existing setups.
