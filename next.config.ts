...
const nextConfig: NextConfig = {
   ...
   *
   * Why this exists:
   *   A shared CDN (a reverse proxy, or a self-hosted deploy behind
   *   Cloudflare) can apply a long `s-maxage` to prerendered HTML by
   *   default. When a new deploy shipped fresh Turbopack chunk
   *   hashes, the edge kept serving stale HTML referencing chunk
   *   filenames that no longer existed on disk — result: HTML 200,
   *   every /_next/static/*.js and .css came back 404, the page
   *   rendered unstyled. Private/incognito did nothing because the
   *   cache is server-side.
   *
   *   Vercel's edge keys its cache on the deployment and invalidates
   *   on every deploy, so it doesn't hit this — but the explicit
   *   header is cheap, self-documenting, and keeps non-Vercel
   *   deploys safe.
   *
   * Strategy:
